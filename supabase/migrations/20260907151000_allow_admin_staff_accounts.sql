...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE user_id = _actor_id AND school_id = _school_id
      AND role IN ('admin', 'principal', 'deputy') AND is_active
  ) THEN
    RAISE EXCEPTION 'Only a school administrator, principal, or deputy can create staff accounts';
  END IF;
  ...
  selected_grades := ARRAY(SELECT value::public.cbe_grade FROM jsonb_array_elements_text(COALESCE(_staff->'assigned_grades', '[]'::jsonb)) AS item(value));
  ...
  IF selected_stream_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM public.streams WHERE id = selected_stream_id AND school_id = _school_id
  ...
  INSERT INTO public.staff (
    school_id, user_id, staff_number, full_name, account_role, tsc_number, national_id, gender, job_title,
    ...
    _school_id, _user_id, generated_staff_number, _staff->>'full_name', _role::text,
    ...
    selected_grades[1], selected_grades, selected_grade, 'active', false, true, now(), 'active', now() + interval '48 hours'
  ) RETURNING id INTO staff_id;

  IF selected_stream_id IS NOT NULL THEN
    UPDATE public.streams SET class_teacher_id = staff_id, updated_at = now()
    WHERE id = selected_stream_id AND school_id = _school_id AND class_teacher_id IS NULL;
    IF NOT FOUND THEN RAISE EXCEPTION 'That stream already has a class teacher'; END IF;
  END IF;
